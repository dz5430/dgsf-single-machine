# -*- coding: utf-8 -*-
"""
Created on Mon May  5 14:52:38 2025
...
df = pd.read_csv("Dev3_singlemachine_instances_80_theta_max_6Itau_u4_raw.csv",encoding='utf-8-sig')
...
print('Dev3_f3_singlemachine_instances_100_theta_max_6Itau_10.csv')
...
for idx, row in df.iterrows():
    ...
    try:
        ...
        if (result.solver.status != pyo.SolverStatus.ok or
            ...
            print(f"⚠️  Instance {idx+1} not optimal.")
            rank_vectors.append(["infeasible"] * I)
            start_time_vectors.append(["infeasible"] * I)
            tardiness_values.append("infeasible")   # ✅ add this
...
#print(len(df), len(rank_vectors), len(start_time_vectors), len(solve_times), len(tardiness_values))
...
df.to_csv("Dev3_singlemachine_instances_80_theta_max_6Itau_u4.csv", index=False)
